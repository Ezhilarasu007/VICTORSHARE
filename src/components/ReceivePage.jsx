...
import { Download, Lock, ShieldCheck, CheckCircle2, Smartphone, ArrowLeft, RefreshCw, Sparkles, HardDrive, Hash, Folder, Image, Film, FileText, Zap, AlertCircle, Clock } from 'lucide-react';
...
export function ReceivePage({ onBackHome, permissions, openPermissionsModal }) {
  ...
  const handleVerifyPin = (presetPin) => {
    ...
    if (pin.replaceAll('-', '').length < 6) {
      setErrorMessage('Please enter a full 6-digit PIN code');
      return;
    }
    ...
    setTimeout(() => {
      ...
      showToast(`P2P Stream Connected! Found ${meta.name}`);
    }, 450);
  };
  ...
  const startFastDownload = () => {
    ...
    const interval = setInterval(() => {
      ...
      // Calculate realistic speed & ETA
      const currentSpeed = (Math.random() * 25 + 95).toFixed(1); // ~110 MB/s fast speed
      ...
      if (pct >= 100) {
        ...
        // AUTO TRIGGER SILENT FILE DOWNLOAD DIRECTLY TO STORAGE (NO ALERT POPUP!)
    ...
    if (completedFile && navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Received ${completedFile.name} via VictorShare P2P`
        });
      } catch (e) {}
    } else {
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto px-4 py-6 relative">
      ...
      {/* Toast Notification Banner (No Browser Popups!) */}
      {toastMessage && (
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-purple-950/80 border border-purple-500/40 text-purple-300 text-xs font-mono font-bold">
          ...
          <span>Direct P2P Stream • No Alerts</span>
        </div>
      </div>
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-xs text-slate-400">Enter the sender's 6-digit PIN code to reveal file details and download directly.</p>
      </div>
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-3xl space-y-8 border border-purple-500/30 text-center max-w-xl mx-auto shadow-2xl">
        ...
        {verifiedFileMeta && (
          <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-950 to-purple-950/60 border-2 border-purple-500/50 text-left space-y-4 animate-fade-in shadow-2xl">
            ...
            {/* Live Progress Bar with Real Speed & ETA */}
            {isReceiving && (
            ...
            {/* Completed Result Actions (NO ALERT POPUPS!) */}
            {completedFile && (
